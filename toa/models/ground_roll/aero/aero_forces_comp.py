...
class AeroForcesComp(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input(name='q', val=np.zeros(nn), desc='Dynamic pressure', units='Pa')
        ...
        self.declare_partials(of='L', wrt='q', rows=ar, cols=ar)
        self.declare_partials(of='D', wrt='CD', rows=ar, cols=ar)
        self.declare_partials(of='D', wrt='q', rows=ar, cols=ar)
        self.declare_partials(of='M', wrt='Cm', rows=ar, cols=ar)
        self.declare_partials(of='M', wrt='q', rows=ar, cols=ar)

    def compute(self, inputs, outputs, **kwargs):
        airplane = self.options['airplane']
        qS = inputs['q'] * airplane.S
