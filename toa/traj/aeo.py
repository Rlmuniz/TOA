...
def run_takeoff(airplane, runway, flap_angle=0.0, wind_speed=0.0):
    ...
    initial_run.set_time_options(fix_initial=True, units='s', duration_bounds=(10, 60))
    ...
    rotation.set_time_options(fix_initial=False, units='s', duration_bounds=(1, 5), duration_ref=5)
    ...
    rotation.add_state(name='h', units='m', rate_source='rotation_eom.h_dot',
                       targets=['mlg_pos.h'], fix_initial=True, fix_final=False, ref=10, defect_ref=10)
    ...
    transition.set_time_options(fix_initial=False, units='s', duration_bounds=(1, 20), duration_ref=10)
    ...
    p['traj.transition.states:gam'] = transition.interpolate(ys=[0.0, 0.0], nodes='state_input')
